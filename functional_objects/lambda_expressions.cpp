...
int main2(){
  ...
  std::sort(v.begin(), v.end(),
    [center](const int a, const int b){ return std::abs(center-a) < std::abs(center-b); } // [ 1 ]
    );
  print(v, "center 50:");
  ...
  auto positiveEvenFirst= [](const int a, const int b)  // [ 2 ]
  {
    if ((a > 0 && b < 0) || (a < 0 && b > 0)) return a > 0;
    if (a == 0 || b == 0) return b < a;
    if (std::abs(a) % 2 == 0 && std::abs(b) % 2 == 1) return true;
    return false;
  };
  ...
  srand(2022);
  int a = 0, b = 40;
  auto generator = [&a, &b]{ return a + rand() % (b-a + 1); };  // [ 3 ]
  ...
  int start = 5, step =2;
  auto arithmeticGenerator =  [start, &step, n = 0]() mutable {return start + (n++) * step; };  // [ 4 ]
