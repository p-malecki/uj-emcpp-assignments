...
void ComputeMandelbrotSet(BMP& output, int width, int height, size_t maxIter,
                          double escapeThreshold, auto pixelToComplex, auto iterationsToPixel) {
    int index = 0;
    std::mutex index_mtx;
    ...
    auto worker = [&]() {
        while (true) {
            ...
            auto numberOfIteration = MandelbrotSetIterations(c, maxIter, escapeThreshold);
...
int main( int argc, char* argv[] )
{
    ...
    ComputeMandelbrotSet(Output, imageWidth, imageHeight, maxNumberOfIteration,
                         escapeThreshold, pixelToComplex, iterationsToPixel);
